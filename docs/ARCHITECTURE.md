...
4. **Black fills take the row-wise clear (`FastMatrixPanel`):** `fillScreen(0)` opens nearly every engine's frame, and the library implements it per pixel (same cost as above, ~100 ms at 256×64), which held the animated clock faces to 7–8 fps. The panel object is a thin subclass that routes black fills to the library's row initialiser (`clearFrameBuffer`, one write-back per row) and re-applies the brightness bits; other colours keep the library path. It needs the back-buffer parity (fed from `MatrixEngine::present()`, the single flip site) and the last applied brightness (fed from `MatrixEngine::setBrightness()`). Measured: clock faces 7–8 → 13–14 fps.
